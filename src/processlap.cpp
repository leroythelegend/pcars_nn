...
namespace pcars
{
    ...
    void ProcessLap::playing(PacketPtr & packet)
    {
        if (packet->type() == PACKETTYPE::PACKETTELEMETRYDATAV1) {
            ...
            // cout << "Lap invalidated                  : " << p->lap_invalidated()               << endl;
            // cout << "Antilock Active                  : " << p->antiLock_active()               << endl;
            // cout << "Boost Active                     : " << p->boost_active()                  << endl;
            ...
            int i = p->viewed_participant_index();
                ...
                // cout << "   Current Lap                : " << p->participant_info().at(i).current_lap()          << endl;
            ...
            if ((p->current_time() != -1)) {
                ...
                if (startrecording_) {
                    cout << endl << "recording lap " << p->participant_info().at(p->viewed_participant_index()).current_lap() << endl << endl;
                }

                if (lapnumber_ != p->participant_info().at(p->viewed_participant_index()).current_lap()) {
                    lapnumber_ = p->participant_info().at(p->viewed_participant_index()).current_lap();
                    if (capturelap_) {
                        if (startrecording_) {
                            cout << "RECORD LAP " << lapnumber_ << endl;
                            cout << "RECORD LAP " << lapnumber_ << endl;
                            cout << "RECORD LAP " << lapnumber_ << endl;
                            cout << "RECORD LAP " << lapnumber_ << endl;
                            cout << "RECORD LAP " << lapnumber_ << endl;
